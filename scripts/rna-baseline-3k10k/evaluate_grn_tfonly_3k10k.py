#!/usr/bin/env python
"""Evaluate the 3k+10k multiome RNA-only baseline (tf_only.txt regulators)
GRNBoost2 network.

Same metrics as the other workflows' evaluate_grn_tfonly.py: retrieval over the
ranked edge list against PBMC-TRRUST.csv and PBMC-Blood.csv (deduplicated).

Reads  results/rna_baseline_3k10k/grn_tfonly_reg/grnboost2_network.tsv
Writes results/rna_baseline_3k10k/grn_tfonly_reg/evaluation/{evaluation_summary.txt,top_edges.csv}
...
def main():
    GRN = f'{PROJ}/results/rna_baseline_3k10k/grn_tfonly_reg'
    ...
    print(f'[rna_baseline_3k10k tf_only] Inferred edges: {len(net)}')
    ...
    def evaluate(gt_name, gt_edges, out):
        ...
        out.write('-' * 70 + '\n')
        return {'total': total, 'dedup': len(dedup), 'recovered': recovered,
    ...
    with open(f'{OUT}/evaluation_summary.txt', 'w') as out:
        out.write('ARBORETO GRNBOOST2 NETWORK EVALUATION SUMMARY '
                  '(RNA-only baseline 3k+10k multiome, tf_only.txt regulators, TRRUST-only targets)\n')
        out.write('=' * 70 + '\n')
