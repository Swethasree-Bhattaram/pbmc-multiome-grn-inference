#!/usr/bin/env python
"""Evaluate the RNA-only baseline tf_only-regulator GRNBoost2 network.

Same metrics as scripts/rna-baseline/evaluate_grn_trrust.py: retrieval over the
ranked edge list against PBMC-TRRUST.csv and PBMC-Blood.csv (deduplicated).

Reads  results/rna_baseline/grn_tfonly_reg/grnboost2_network.tsv
Writes results/rna_baseline/grn_tfonly_reg/evaluation/{evaluation_summary.txt,top_edges.csv}
...
def main():
    GRN = f'{PROJ}/results/rna_baseline/grn_tfonly_reg'
    ...
    print(f'[rna_baseline tf_only] Inferred edges: {len(net)}')
    ...
    with open(f'{OUT}/evaluation_summary.txt', 'w') as out:
        out.write('ARBORETO GRNBOOST2 NETWORK EVALUATION SUMMARY '
                  '(RNA-only baseline, tf_only.txt regulators, TRRUST-only targets)\n')
        out.write('=' * 70 + '\n')
