#!/usr/bin/env python
"""Evaluate the RNA-only baseline GRNBoost2 network against PBMC ground truth.

Same metrics as the other workflows' evaluate_grn_trrust.py: retrieval over the
ranked edge list against PBMC-TRRUST.csv and PBMC-Blood.csv (deduplicated).

Reads  results/rna_baseline/grn_trrust/grnboost2_network.tsv
Writes results/rna_baseline/grn_trrust/evaluation/{evaluation_summary.txt,top_edges.csv}
...
def main():
    GRN = f'{PROJ}/results/rna_baseline/grn_trrust'
    ...
    print(f'[rna_baseline] Inferred edges: {len(net)}')
    ...
    with open(f'{OUT}/evaluation_summary.txt', 'w') as out:
        out.write('ARBORETO GRNBOOST2 NETWORK EVALUATION SUMMARY '
                  '(RNA-only baseline, TRRUST-only targets)\n')
        out.write('=' * 70 + '\n')
