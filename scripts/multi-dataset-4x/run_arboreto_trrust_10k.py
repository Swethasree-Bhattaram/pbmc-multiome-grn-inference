#!/usr/bin/env python
"""Run Arboreto GRNBoost2 with TRRUST-only target genes (full-10k).

Variant where TARGET genes are restricted to ONLY the TRRUST TFs present in the
expression matrix (no top-2000 HVG). Reuses the existing all-cells matrix
(results/<exp>_10k/all_cells_gene_expression.npy) and writes to a SEPARATE
folder results/<exp>_10k/grn_trrust/.
...
def main():
    exp = sys.argv[1]
    DOWN = f'{PROJ}/results/{exp}_10k'
    OUT = f'{PROJ}/results/{exp}_10k/grn_trrust'
