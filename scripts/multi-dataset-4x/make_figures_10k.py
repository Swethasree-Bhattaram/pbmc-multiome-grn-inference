#!/usr/bin/env python
"""Generate diagnostic figures for the full-10k experiments.

Reads results/exp*_10k/grn_trrust evaluations + integration, writes figures to
reports/figures_10k. Run with the scSAGA .venv.
"""
...
RES = f'{PROJ}/results'
REP = f'{PROJ}/reports/figures_10k'
...
def load_eval(exp):
    p = f'{RES}/{exp}_10k/grn_trrust/evaluation/evaluation_summary.txt'
    d = {}
...
def main():
    ...
    for gt in gts:
        ...
        plt.title(f'GRN edges recovered — {gt} (full-10k)')
        plt.ylabel('deduplicated ground-truth edges found')
        plt.tight_layout(); plt.savefig(f'{REP}/recovered_{gt}.png', dpi=150); plt.close()

    # mean imputed expression density (mmap to avoid loading full matrix)
    means = []
    for e in EXPS:
        imp = np.load(f'{RES}/{e}_10k/imputed_expression_genes_x_atac.npy', mmap_mode='r')
        means.append(float(np.asarray(imp).mean()))
    plt.figure(figsize=(6.5, 4))
    plt.bar(EXPS, means, color=[COLORS[e] for e in EXPS])
    plt.title('Mean reverse-imputed ATAC expression (full-10k)')
    plt.ylabel('mean log1p(CPM)')
    plt.tight_layout(); plt.savefig(f'{REP}/imputed_mean.png', dpi=150); plt.close()

    # joint embedding 2D
    j2 = f'{RES}/integration_10k/joint_embedding_2d.csv'
    if os.path.exists(j2):
        df = pd.read_csv(j2)
        plt.figure(figsize=(7, 6))
        for ds, clr in [('rna3k','#2c3e50'),('atac3k','#c0392b'),('rna10k','#27ae60'),('atac10k','#8e44ad')]:
            sub = df[df.dataset == ds]
            plt.scatter(sub.x, sub.y, s=3, label=ds, color=clr, alpha=0.6)
        plt.legend(markerscale=3)
        plt.title('scSAGA 4-dataset joint embedding (2D, full-10k)')
