#!/usr/bin/env python
"""Evaluate an experiment's Arboreto GRNBoost2 network against PBMC ground-truth
regulatory edges (PBMC-TRRUST, PBMC-Blood). Same metrics as the original 3k eval.

For each ground-truth set reports:
  - Total / Deduplicated edges
  - Edges recovered by GRN (anywhere in the network)
  - Precision@K / Recall@K for top-K inferred edges

Run with the scSAGA .venv (numpy/pandas).
Usage: python evaluate_grn.py <exp>
"""
import sys, os, csv
import pandas as pd
...
def main():
    exp = sys.argv[1]
    GRN = f'{PROJ}/results/{exp}/grn'
    OUT = f'{PROJ}/results/{exp}/grn/evaluation'
    ...
    with open(f'{OUT}/evaluation_summary.txt', 'w') as out:
        out.write(f'ARBORETO GRNBOOST2 NETWORK EVALUATION SUMMARY (exp {exp})\n')
        out.write('=' * 70 + '\n')
