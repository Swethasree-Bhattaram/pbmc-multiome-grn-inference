...
(full-10k). Reads results/<exp>_10k/grn_trrust/, writes evaluation there.

Run with the scSAGA .venv (numpy/pandas).
Usage: python evaluate_grn_trrust_10k.py <exp>
"""
import sys, os, csv
import pandas as pd
...
def main():
    exp = sys.argv[1]
    GRN = f'{PROJ}/results/{exp}_10k/grn_trrust'
    OUT = f'{PROJ}/results/{exp}_10k/grn_trrust/evaluation'
    ...
    with open(f'{OUT}/evaluation_summary.txt', 'w') as out:
        out.write(f'ARBORETO GRNBOOST2 NETWORK EVALUATION SUMMARY (exp {exp}, TRRUST-only targets, full-10k)\n')
        out.write('=' * 70 + '\n')
