#!/usr/bin/env python
"""Generate a consolidated comparison report for the full-10k TRRUST-only GRN runs.

Reads results/exp{A,B1,B2}_10k/grn_trrust/ (networks + evaluations) and writes
reports/comparison_report_trrust_10k.md. Same metrics/format as the 6k comparison
but for the full-10k (11,898-cell) datasets.
...
EXP_META = {
    'expA':  dict(label='Experiment A', title='Reverse-imputeKNN from a SCEMENT-integrated combined RNA reference',
                  ref='SCEMENT-integrated 3k+10k RNA (14,609 cells)'),
...
def main():
    ...
    L = []
    L.append('# Multi-Dataset scSAGA Integration & GRN: TRRUST-only Target Comparison (full-10k)')
    L.append('')
    L.append('## Design')
    L.append('')
    L.append('Same four-dataset scSAGA integration and reverse-imputeKNN experiments as the main')
    L.append('comparison, but using the **full 10k multiome (11,898 cells)** instead of a 2,711-cell')
    L.append('subsample, and the Arboreto GRNBoost2 **target genes are restricted to ONLY the TRRUST')
    ...
    report = '\n'.join(L)
    with open(f'{REP}/comparison_report_trrust_10k.md', 'w') as f:
        f.write(report)
    print('Wrote', f'{REP}/comparison_report_trrust_10k.md')
