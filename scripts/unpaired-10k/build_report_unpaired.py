...
def main():
    ...
    E = json.load(open(f'{GRN}/evaluation.json'))
...
| Mean NN distance query→reference | {Q['corr_true_nbr']:.4f} corr to assigned neighbours; {QM['distinct_ref_cells_used']:,}/11,898 reference cells used |
...
|---|---|
| Cells | {E['n_cells']:,} |
| **Target genes** | genes present in `trrust_tf.txt` → **{E['n_targets']:,}** (TRRUST list 2,862) |
| **Regulators** (`tf_names`) | TFs present in `tf_only.txt` → **{E['n_regulators']:,}** (list 829) |
...
§5 (r=+0.741 vs +0.485 control) is the informative evidence.

### Consequences for the GRN

- 8,161 of 20,059 cells (**40.7%**) are imputed, so the majority of rows are
