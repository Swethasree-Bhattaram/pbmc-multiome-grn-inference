#!/usr/bin/env python
"""Run Arboreto GRNBoost2 on the all-cells expression matrix using TRRUST TFs.

Same integration pipeline. Consumes results/downstream, writes to results/grn.
Requires the .venv39 python 3.9 env (dask 2021.10, arboreto 0.1.6).
Run inside if __name__ == '__main__' guard for macOS multiprocessing.
"""
...
def main():
    ...
    network = grnboost2(
        expression_data=expr_sub,
        tf_names=present_tfs,
        client_or_address=client,
        verbose=False,
        seed=666,
    )
    print('GRNBoost2 done. Edges:', len(network), flush=True)
    print(network.head(), flush=True)

    network.to_csv(f'{OUT}/grnboost2_network.tsv', sep='\t', index=False)
    network.to_csv(f'{OUT}/grnboost2_network.csv', index=False)
    with open(f'{OUT}/tf_regulators.txt','w') as f: f.write('\n'.join(present_tfs))
    with open(f'{OUT}/target_genes.txt','w') as f: f.write('\n'.join(target_genes))
    print('Saved grnboost2_network.tsv / .csv, tf_regulators.txt, target_genes.txt', flush=True)
